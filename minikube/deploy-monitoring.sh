...
if ! minikube status &>/dev/null; then
  ...
  echo "  ./minikube/setup-minikube.sh"
  exit 1
fi
...
# Get the Minikube IP for accessing services
...
echo "  - OTLP gRPC: ${MINIKUBE_IP}:${OTLP_GRPC_PORT:-4317}"
echo "  - OTLP HTTP: ${MINIKUBE_IP}:${OTLP_HTTP_PORT:-4318}"
echo ""
